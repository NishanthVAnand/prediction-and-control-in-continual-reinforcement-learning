...
best_lrs = {'TD-Res': (0.03, None), 'TD-Cont': (0.03, None), 'PT-Mem': (0.0003, 0.01)}
...
for algo, (b_lr1, b_lr2) in best_lrs.items():
    ...
    plt.plot(online_err_mean, c=colors[algo], alpha=0.75)
    ...
    plt.plot(others_err_mean, c=colors[algo], ls="--", lw=0.7, alpha=0.75)
...
plt.legend(custom_lines, ["Deep TD", "Deep TD (reset)", "PT-Deep TD (ours)", "Other Tasks"], ncol=4, fontsize=14, loc="upper right", bbox_to_anchor=(0.2, 0., 0.8, 1.0), frameon=True)
plt.xlabel("Episodes", fontsize=14)
plt.ylabel("RMSVE", fontsize=14)
plt.tick_params(axis='both', which='major', labelsize=13)
plt.title("Image Grid (DNN)", fontsize=14)

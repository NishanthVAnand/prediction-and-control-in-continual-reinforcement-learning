...
best_lrs = {'TD-Res': (0.3, None), 'TD-Cont': (0.3, None), 'PT-Mem': (0.01, 0.3)}
...
pdf = matplotlib.backends.backend_pdf.PdfPages("plots/DiscreteGrid_tabular_error_curves.pdf")
fig, ax = plt.subplots(figsize=(12,4))
colors = {"PT-Mem":'b', "TD-Cont":'g', "TD-Res":'r'}
for algo, (b_lr1, b_lr2) in best_lrs.items():
    if algo == "PT-Mem":
        fname = "PT_Mem_ntasks_4_gamma_"+str(gamma)+"_feat_gridTabular_episodes_"+str(episodes)+"_switch_"+str(switch)+"_lr1_"+str(b_lr1)+"_lr2_"+str(b_lr2)
    elif algo == "TD-Cont":
        fname = "TD"+"_reset_"+str(False)+"_ntasks_4_gamma_"+str(gamma)+"_feat_gridTabular_episodes_"+str(episodes)+"_switch_"+str(switch)+"_lr1_"+str(b_lr1)
    elif algo == "TD-Res":
        fname = "TD"+"_reset_"+str(True)+"_ntasks_4_gamma_"+str(gamma)+"_feat_gridTabular_episodes_"+str(episodes)+"_switch_"+str(switch)+"_lr1_"+str(b_lr1)
    ...
    plt.plot(seed_ot_mean, c=colors[algo], alpha=0.75, ls="--", lw=0.5)
...
plt.title("Discrete Grid (Tabular)", fontsize=14)

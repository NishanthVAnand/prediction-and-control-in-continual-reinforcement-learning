...
parser.add_argument('--k', type=int, default=1, help="frequency of permanent memory updates (episodes)")
...
def update_w1(w_1, replay):
	...
	for c_f_1, c_f_2 in replay:
		w_1 += lr1 * (old_w1.dot(c_f_1) + w_2.dot(c_f_2) - w_1.dot(c_f_1)) * c_f_1
	return w_1
...
filename = "PT_ql_crl"+"_ntasks_"+str(len(envs))+"_gamma_"+misc_param['gamma']+"_feat_"+args.feat_type+\
		"_episodes_"+str(args.t_episodes)+"_switch_"+str(args.switch)+"_decay_"+str(args.decay)+"_lr1_"+str(args.lr1)+"_lr2_"+str(args.lr2)+\
		"_k_"+str(args.k)

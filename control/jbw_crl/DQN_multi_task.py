...
def train_Net(step):
	...
	with torch.no_grad():
		...
		next_pred = next_pred[(step//150000)%2].max(1)[0]
	pred = Net(states)
	pred = pred[(step//150000)%2].gather(1, actions)
...
def get_action(c_obs, step):
	...
	if np.random.random() <= epsilon:
		...
		action = curr_Q_vals[(step//150000)%2].max(1)[1].item()
	return curr_Q_vals[(step//150000)%2][0][action], action
...
filename = "DQN_multi_task"+"_gamma_"+misc_param['gamma']+\
		"_steps_"+str(args.t_steps)+"_batch_"+str(args.batch_size)+\
		"_lr1_"+str(args.lr1)+"_seed_"+str(args.seed)+"_ftype_"+args.ftype

Net = NN_FA_two_heads(env.feature_space.shape[0]*env.feature_space.shape[1], env.action_space.n).to(device)
opt = optim.Adam(Net.parameters(), lr=args.lr1)
criterion = torch.nn.MSELoss()

Target_net = NN_FA_two_heads(env.feature_space.shape[0]*env.feature_space.shape[1], env.action_space.n).to(device)
